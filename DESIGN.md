...
### 模态
`--z-modal`，圆角 `--r-xl`，`box-shadow: var(--e4)`，`transform-origin: center`。**模态要不要"从触发器长出来"，由触发场景决定**，不再一律豁免：表单类模态（插入链接、存储位置、确认框）与触发器没有空间关系，保持居中弹出；当模态承载的是**列表里某一件东西的展开**时，它改为从那一件所在的位置长出来——否则用户会丢掉"我点的是哪一件"这条线索。**品味库的详情弹窗属于后者**：它从被点击的那张卡片位置展开，`transform-origin` 因此不落在自身中心。
