...
本条管的是图标按钮，以及一次点击完成一件事的独立按钮。**横排成一条的密集文字胶囊**（`.tag` / `.tchip` / 品味库的筛选胶囊）按按钮分档里的 `28px` 那一档走：它们的命中区就是视觉区（`height: 28px` + `padding: 0 13px`）。撑到 44px 会把整条筛选行变成一条 44px 高的横带，胶囊之间也不再是一排能互相比较的对象 —— 这是分档表里留了 `28px` 一档的原因。
...
### 空态
必须给一个**明确的下一步**（`baseline-ui`），不是一句"暂无数据"。本站的落法是 `.state-empty`（`<strong>` 说结论 + `<p>` 说下一步），每个视图各自有针对性文案：
...
| 品味库·没连文件夹 | 还没有指定存放的位置 | 品味库把缩略图存在你指定的文件夹里。先选一个文件夹，再开始收藏。 |
| 品味库·库是空的 | 还没有收藏任何东西 | 点下面那张虚线卡片，把第一个网页或一段备忘收进来。 |
| 品味库·筛选无果 | 没有找到符合条件的收藏 | 换个关键词，或清空筛选。 |

品味库那三行的措辞与翻阅刻意不同（**收藏** / 收藏夹，不是「日记」）：库里放的是收藏，说成日记会让人以为筛错了库。品味库的「没连文件夹」「筛选无果」两套在文案下面还有一颗按钮（`选择文件夹` / `清空筛选`）——空态给的是"下一步"，能点的那一步最省事。
...
### 模态
`--z-modal`，圆角 `--r-xl`，`box-shadow: var(--e4)`，`transform-origin: center`。**模态要不要"从触发器长出来"，由触发场景决定**，不再一律豁免：表单类模态（插入链接、存储位置、确认框）与触发器没有空间关系，保持居中弹出；当模态承载的是**列表里某一件东西的展开**时，它改为从那一件所在的位置长出来——否则用户会丢掉"我点的是哪一件"这条线索。**品味库的详情弹窗属于后者**：它从被点击的那张卡片位置展开，`transform-origin` 因此不落在自身中心。它的缩放起点用 `.42`（远小于下面"不要用 `scale(0)`"那条给的 `.95–.97`）：那条禁的是"没有空间关系的居中弹窗配上夸张缩放"，而这一枚的起点就是被点的那张卡片本身，放大过程正是"我点的那一件铺开"的线索——不这么做，用户会丢掉自己点的是哪一件。**减少动效时，出场那一段也要一起压平**：`.modal-box.out` 带了 `!important` 来压基类的 `popIn`，样式表末尾那条全局 `*{animation-duration:.01ms!important}` 因此压不过它，得在同特异度但更靠后的位置单独写一条（见 `index.html` 样式表里的详情弹窗段）。
≤640px 时详情弹窗**铺满整屏**（`border-radius:0`、遮罩内边距归零）：它是"某一件事物被打开"，不是居中的对话框，手机上没有让它悬在中间的理由。
...
**写死颜色的地方之一**：关闭按钮的 `hsl(0 0% 100% / .12)` 底 + `hsl(0 0% 100% / .9)` 字。它叠在任意照片上，跟随主题令牌会在浅色照片上消失（同类例外都记在下面的「明确豁免」里）。
...
- **渐变**：`.scrim` 与标题区遮罩使用渐变，因为它们是**调节背景亮度的工具**，不是装饰。授权来自用户对 Q19 的选择。标题区那条白纱（`.view-head::before`）向左右各溢出 `28px` 做柔边，靠的是桌面 `main` 的内边距比这个宽；`≤960px` 时 `main` 的内边距只有 `14px`，它就会顶出视口、让整页能左右拖，所以那一档收到 `14px`（贴着屏幕边，不出界）。
...
- **下拉箭头的写死色值**：`.tool-select` 与 `.taste-select` 的箭头是数据 URI 里的内联 SVG（原生 `select` 不能带伪元素，画箭头的唯一办法是 `background-image`），而那里的色值引用不了 CSS 变量，只能写死 `#7a8fa3`。两处必须保持一致；要改色就两处一起改，或者换成别的画法（真 `<svg>` + 包裹元素）。
- **叠在照片上的药丸与角标**：品味库详情弹窗右下角那张 `1 / 2` 计数药丸（`.taste-detail-count`）、卡片缩略图左上角那颗「待落盘」角标（`.taste-pending-badge`）、lightbox 的关闭按钮同因——它们压在任何一张照片上，跟随主题令牌会在浅色照片上消失。写死 `hsl(212 34% 10% / .62)` 底 + `hsl(208 30% 96%)` 字，三处同源，要改一起改。
