...
- 清理浏览器数据、换浏览器或换电脑都会丢失，建议定期「导出备份」，或者连接一个本地文件夹让每次保存自动落盘
...
**换了电脑，日记怎么不见了？**
日记存在浏览器本地，需要先在旧设备「导出备份」，再在新设备「导入备份」；用过文件夹同步的话，直接在新设备重连同一个文件夹也可以。
...
**点「存储」提示不支持？**
文件夹同步需要桌面版 Edge / Chrome。其它浏览器请用「导出备份」手动存盘。
